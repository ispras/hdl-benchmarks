...
AG((invalidate=1)->AX((pcacheA.state=D)+(pcacheB.state=D)+(pcacheC.state=D)));
...
AG((pcacheA.state=S * pcacheA.t_info_avail = 1) -> ((pcacheB.t_info_avail=1)+(pcacheC.t_info_avail=1)));
...
AG(((pcacheA.state=S)*(pcacheA.hit=1))->((pcacheB.t_info_avail=1) + (pcacheC.t_info_avail=1)));
# A variant of the above property

AG(((pcacheA.write=1) && (pcacheB.t_info_avail=1))->AF(pcacheB.state = I));
...
AG(((pcacheA.write=1)*(bus_ackA=1)*(pcacheA.hit=1)*(is_snoop=0))->(AX(pcacheA.state= D))); 
# On an write hit signal, the processor should update it's state to Dirty

#AG(((shared_snoop=SHW)*(pcacheA.state=S)*(is_snoop=1)*(bus_ackA=0))->((pcacheA.t_info_avail=1)));
...
AG(((pcacheA.state=D)*(is_snoop=1)*(pcacheA.info_avail=1)*(bus_reqA=0))->(pcacheA.next_state=S + pcacheA.state=I));
