...
AG((pcacheA.state=S * pcacheA.t_info_avail = 1 * pcacheB.state=S) -> ((pcacheB.t_info_avail=1)));
...
AG(((pcacheA.write=1)*(pcacheA.hit=1))->(AX(pcacheA.state= D))); 
# On an write hit signal, the processor should update it's state to Dirty
#Property fails
AG(((pcacheB.write=1)*(pcacheB.hit=1))->(AX(pcacheB.state= D))); 
#Property fails
...
AG(((pcacheA.state=D)*(is_snoop=1)*(pcacheA.t_info_avail=1)*(bus_reqA=0))->AX(pcacheA.state=S + pcacheA.state=I));
...
AG((invalidate=1)->AX((pcacheA.state=D)+(pcacheB.state=D)));
